...
# Create Logstash manifest
resource "kubectl_manifest" "logstash_demo" {
  yaml_body = file("manifest/000-elasticsearch.yaml")
  provisioner "local-exec" {
    command = "sleep 30"
  }
  depends_on = [time_sleep.wait_30_seconds]
...
resource "kubectl_manifest" "logstash_config" {
  yaml_body = file("manifest/001-configmap.yaml")
  provisioner "local-exec" {
    command = "sleep 30"
  }
  depends_on = [time_sleep.wait_30_seconds]
...
resource "kubectl_manifest" "logstash_deployment" {
  yaml_body = file("manifest/002-deployment.yaml")
  provisioner "local-exec" {
    command = "sleep 30"
  }
  depends_on = [time_sleep.wait_30_seconds]
...
resource "kubectl_manifest" "logstash_service" {
  yaml_body = file("manifest/003-service.yaml")
  provisioner "local-exec" {
    command = "sleep 30"
  }
  depends_on = [time_sleep.wait_30_seconds]
